...
2. Run KGFlow with the corresponding connection parameters:
   ...
   The release uses local Hugging Face-compatible model weights exclusively. Connection URLs, usernames, and passwords are required command-line arguments so that deployment-specific values are never stored in the repository. `--sample_workers` controls sample-level concurrency; local KG workers must remain single-process because each worker loads the model.

   `--max_candidate_paths`, `--max_alignment_entities`, and `--max_alignment_pairs_per_entity` bound the most expensive downstream KG fusion/alignment stages.
